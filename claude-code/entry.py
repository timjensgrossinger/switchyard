...
def _resolve_provider() -> ClaudeCodeProvider:
    registry = get_registry()
    registry.register_adapter(adapter_from_legacy())
    adapter = registry.resolve_adapter("claude", ProviderCapability.EXECUTE)
    if adapter is None:
        log.warning("Claude adapter unavailable; falling back to direct provider")
        return ClaudeCodeProvider()
    try:
        provider = adapter.invoke("build_provider")
    except Exception:
        log.warning("Claude adapter build_provider failed; falling back", exc_info=True)
        return ClaudeCodeProvider()
    return provider if isinstance(provider, ClaudeCodeProvider) else ClaudeCodeProvider()
