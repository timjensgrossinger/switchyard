...
Per D-01, D-02, D-05, D-11, D-12 (first-class adapter, workspace-write, strict detection, registry+thin-entry, graceful degradation).
...
def _resolve_provider():
    """Resolve Cursor provider via registry.
    ...
    try:
        ...
        adapter = registry.resolve_adapter("cursor", ProviderCapability.EXECUTE)
        if adapter is not None:
            try:
                provider = adapter.invoke("build_provider")
                log.info("Cursor adapter resolved from registry")
                return provider
            except Exception as e:
                log.debug("Cursor adapter invoke failed: %s", e)
    except Exception as e:
        log.debug("Cursor registry resolution failed: %s", e)

    log.debug("Cursor provider unavailable; continuing without provider")
    return None
...
def cmd_route(task: str) -> None:
    ...
    provider = _resolve_provider()
    model = provider.resolve_model(decision.tier) if provider else "claude-sonnet"
