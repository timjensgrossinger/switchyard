...
from opencode.providers import OPENCODE_LOW_MODEL
...
os.environ.setdefault("OPENCODE_HOST", "1")

log = logging.getLogger(__name__)


def _resolve_provider():
    """Resolve OpenCode provider via registry."""
    try:
        registry = get_registry()
        registry.register_adapter(adapter_from_legacy())
        adapter = registry.resolve_adapter("opencode", ProviderCapability.EXECUTE)
        if adapter is not None:
            try:
                provider = adapter.invoke("build_provider")
                log.info("OpenCode adapter resolved from registry")
                return provider
            except Exception as exc:
                log.debug("OpenCode adapter invoke failed: %s", exc)
    except Exception as exc:
        log.debug("OpenCode registry resolution failed: %s", exc)

    log.debug("OpenCode provider unavailable; continuing without provider")
    return None
